...
/// Shortcut for Log Scenario Messages
function logScenario(str) {
    UIALogger.logMessage("⭑ SCENARIO > " + str);
}
...
function performTask(tasks) {
    ...
    for (var i in tasks) {
        ...
        // Starting The Scenario //
            ...
            if (!missing) {
                logSucess(step[step.length - 1]);
            }
            else if (missing == FailTest) {
                logFail(step[step.length - 1]);
                GlobalEnv.Pass = false;
                break;
            }
        }
    }
    if (missing != FailTest && missing) {
        logIssue("[error] cannot find or interact with "+ missing + " during step: " + step);
        logIssue("[failure investigation] current elements on screen");
        logTree(win());
        logFail("fail to find or interact with "+ missing + " during step: " + step);
        // throw "fail";
        GlobalEnv.Pass = false;
    }
}
