...
function performTask(tasks) {
    ...
    if (missing) {
        log("[error] cannot find or interact with "+ missing + " during step: " + step);
        log("[failure investigation] current elements on screen");
        logTree(win());
        UIALogger.logFail("fail to find or interact with "+ missing + " during step: " + step);
        // throw "fail";
